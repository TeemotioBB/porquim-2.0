...
from src.core.database import (
    buscar_gastos_mes, buscar_entradas_mes,
    total_gasto_mes, total_entrada_mes,
    buscar_limite, salvar_limite
)
...
async def gerar_resumo(usuario: str, ano: int = None, mes: int = None) -> tuple[str, list]:
    ...
    if not gastos and not entradas:
        return (
            f"📊 *Resumo de {periodo}*\n\n"
            f"Nenhuma transação registrada em {periodo}. 🙌",
            []
        )
    ...
    if entradas:
        linhas_ent = []
        for i, e in enumerate(entradas[:5]):
            emoji = EMOJI_ENTRADA.get(e["categoria"], "💰")
            ...
            linhas_ent.append(f"{num} {emoji} {e['descricao']} · R$ {float(e['valor']):.2f} · {data_fmt}")
        bloco_entradas = (
            f"\n\n💚 *Entradas do mês:* R$ {total_entradas:.2f}\n"
            + "\n".join(linhas_ent)
        )

    # ── Bloco de gastos por categoria ────────────────────────────────────────
    bloco_cats = ""
    if gastos:
    ...
    bloco_saldo = f"\n\n{emoji_saldo} *Saldo do mês:* R$ {saldo:+.2f}"
    ...
    resumo = (
        ...
        + bloco_cats
        + bloco_limite
        + (f"\n\n*Últimos gastos:*\n" + "\n".join(linhas_gastos) if gastos else "")
