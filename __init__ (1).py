...
CATEGORIAS_GASTO   = "Alimentação, Transporte, Vestuário, Moradia, Saúde, Educação, Lazer, Outros"
CATEGORIAS_ENTRADA = "Salário, Freelance, Investimento, Presente, Reembolso, Outros"
HOJE = str(date.today())
...
Você é o Porquim, assistente financeiro brasileiro.
O usuário está registrando uma ENTRADA (receita/renda), não um gasto.
Analise a mensagem e extraia em JSON:
- valor: número float (ex: 3000.00)
- descricao: descrição limpa da entrada (ex: "Salário", "Freela site", "Dividendos")
- categoria: uma de ({categorias})
- forma_pagamento: Pix, Transferência, Dinheiro ou Desconhecido
- data: data no formato DD-MM-YYYY. Hoje é {hoje}. Se não mencionar data use {hoje}.
...
async def processar_gasto_texto(texto: str) -> dict:
    ...
    prompt = PROMPT_EXTRACAO.format(
        categorias=CATEGORIAS_GASTO, hoje=HOJE, texto=texto
    )
...
async def processar_entrada_texto(texto: str) -> dict:
    """Extrai dados de uma entrada (receita) de uma mensagem de texto."""
    prompt = PROMPT_EXTRACAO_ENTRADA.format(
        categorias=CATEGORIAS_ENTRADA, hoje=HOJE, texto=texto
    )
    resp = await grok.chat.completions.create(
        model="grok-4-1-fast-non-reasoning",
        messages=[{"role": "user", "content": prompt}],
...
- categoria: uma de ({CATEGORIAS_GASTO})
- forma_pagamento: Pix, Cartão, Dinheiro ou Desconhecido
