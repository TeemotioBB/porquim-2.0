...
💚 *REGISTRAR ENTRADA*
━━━━━━━━━━━━━━━━━━
Use a palavra *recebi*, *entrada* ou *salário*:
- _"recebi salário 3000"_
- _"entrada freela 800 pix"_
- _"salário 5000"_
- _"recebi dividendos 250"_

━━━━━━━━━━━━━━━━━━
📊 *VER RELATÓRIOS*
━━━━━━━━━━━━━━━━━━
- *resumo* → mês atual (gastos + entradas + saldo)
...
_Receita salva com sucesso!_ 🎉
_Para remover responda: *remover*_"""
...
# Palavras-chave que sinalizam claramente uma ENTRADA
_ENTRADA_KEYWORDS = re.compile(
    r"\b(recebi|salário|salario|entrada|freela|freelance|dividendo|rendimento|"
    r"reembolso|receita|renda|pagamento\s+receb|transferência\s+receb|pix\s+receb)\b",
    re.IGNORECASE
)

async def _detectar_intencao(texto: str) -> str:
    """
    Retorna: GASTO | ENTRADA | OUTRO
    """
    t = texto.strip().lower()

    # ── Heurística rápida: textos claramente não-transação ────────────────────
    ...
    # ── Detecção rápida de ENTRADA via palavras-chave ─────────────────────────
    if _ENTRADA_KEYWORDS.search(texto):
        return "ENTRADA"

    # Padrão "salário 5000" sem a palavra recebi
    if re.search(r"\b(salario|salário)\b", t) and re.search(r"\b\d+", t):
        return "ENTRADA"

    # Apenas um número solto → não é transação
    if re.search(r"\b\d+([.,]\d+)?\b", t) and len(t.split()) == 1:
        return "OUTRO"
    ...
    try:
            ...
            messages=[
                {
                    ...
                    "content": (
                        ...
                        "Responda APENAS com uma palavra: GASTO, ENTRADA ou OUTRO.\n\n"
                        "GASTO = despesa financeira. Ex: 'uber 25', 'mc donalds 45 cartão', 'gasolina 90 pix'.\n"
                        "ENTRADA = receita/renda recebida. Ex: 'recebi 3000', 'salário 5000', 'freela 800', "
                        "'entrada pix 1200', 'dividendos 250'.\n"
                        "OUTRO = qualquer outra coisa: saudações, perguntas, piadas, números aleatórios sem contexto."
                    ),
                },
        ...
        return resultado if resultado in ("GASTO", "ENTRADA", "OUTRO") else "OUTRO"
...
async def handle_text_message(message: dict) -> dict:
    ...
    if texto_lower == "remover":
        ...
        if ok:
            ...
            tipo_label = "Entrada" if g.get("tipo") == "entrada" else "Gasto"
            return {"type": "text", "content": f"🗑️ *{tipo_label} removido!*\n\n_{g['descricao']} · R$ {float(g['valor']):.2f}_"}
        return {"type": "text", "content": "❌ Não consegui remover. Tente novamente."}
    ...
    # ── Detecção de intenção → Registrar gasto ou entrada ────────────────────
    intencao = await _detectar_intencao(texto)

    if intencao == "ENTRADA":
        try:
            dados = await processar_entrada_texto(texto)
            gasto_id = await salvar_gasto(numero, dados, fonte="texto", tipo="entrada")
            ...
            return {
                ...
                "content": (
                    ...
                    "_'recebi salário 3000'_ ou _'freela 800 pix'_\n\n"
    ...
    if intencao == "GASTO":
        try:
            ...
            gasto_id = await salvar_gasto(numero, dados, fonte="texto", tipo="gasto")
            _ultimo_gasto[numero] = gasto_id
